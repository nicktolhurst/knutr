...
public interface IMessagingService
{
    ...
    /// <param name="blocks">Optional Block Kit blocks.</param>
    /// <param name="ct">Cancellation token.</param>
    Task PostEphemeralAsync(string channelId, string userId, string text, object[]? blocks = null, string? threadTs = null, CancellationToken ct = default);

    /// <summary>
    /// Fetches a message by channel and timestamp, returning its text and thread context.
    /// </summary>
    Task<FetchedMessage?> FetchMessageAsync(string channelId, string messageTs, CancellationToken ct = default);
}

public sealed record FetchedMessage(string Text, string? ThreadTs);
