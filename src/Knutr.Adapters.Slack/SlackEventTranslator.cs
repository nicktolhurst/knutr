...
public static class SlackEventTranslator
{
    ...
    public static bool TryParseReaction(JsonElement root, out ReactionContext? ctx)
    {
        ctx = null;
        if (!root.TryGetProperty("event", out var ev)) return false;
        var type = ev.GetProperty("type").GetString();
        if (type != "reaction_added") return false;

        var team = root.TryGetProperty("team_id", out var t) ? t.GetString() ?? "" : "";
        var user = ev.TryGetProperty("user", out var u) ? u.GetString() ?? "" : "";
        var reaction = ev.TryGetProperty("reaction", out var r) ? r.GetString() ?? "" : "";

        if (!ev.TryGetProperty("item", out var item)) return false;
        var channel = item.TryGetProperty("channel", out var ch) ? ch.GetString() ?? "" : "";
        var itemTs = item.TryGetProperty("ts", out var its) ? its.GetString() ?? "" : "";

        var itemUser = ev.TryGetProperty("item_user", out var iu) ? iu.GetString() ?? "" : "";
        var correlationId = Guid.NewGuid().ToString("N")[..12];

        ctx = new("slack", team, channel, user, reaction, itemUser, itemTs, correlationId);
        return true;
    }
