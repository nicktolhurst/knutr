    ...
    ILogger<SlackThreadedMessagingService> log) : IMessagingService
{
    ...
    public async Task PostEphemeralAsync(string channelId, string userId, string text, object[]? blocks = null, string? threadTs = null, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(_opts.BotToken))
        {
    ...
    public async Task<FetchedMessage?> FetchMessageAsync(string channelId, string messageTs, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(_opts.BotToken))
        {
            log.LogWarning("Slack BotToken not configured, cannot fetch message");
            return null;
        }

        // Try conversations.history first (works for top-level messages)
        var result = await FetchViaHistoryAsync(channelId, messageTs, ct);
        if (result is not null) return result;

        // Fall back to conversations.replies (works for thread replies)
        return await FetchViaRepliesAsync(channelId, messageTs, ct);
    }

    private async Task<FetchedMessage?> FetchViaHistoryAsync(string channelId, string messageTs, CancellationToken ct)
    {
        var url = $"{_opts.ConversationsHistoryUrl}?channel={channelId}&latest={messageTs}&oldest={messageTs}&inclusive=true&limit=1";
        ...
        if (!res.IsSuccessStatusCode)
        {
            log.LogWarning("Slack conversations.history failed: {Status} {Body}", res.StatusCode, body);
            return null;
        }
    ...
    private async Task<FetchedMessage?> FetchViaRepliesAsync(string channelId, string messageTs, CancellationToken ct)
    {
        var url = $"{_opts.ConversationsRepliesUrl}?channel={channelId}&ts={messageTs}&latest={messageTs}&oldest={messageTs}&inclusive=true&limit=1";
        using var req = new HttpRequestMessage(HttpMethod.Get, url);
        req.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", _opts.BotToken);
        ...
        if (!res.IsSuccessStatusCode)
        {
            log.LogWarning("Slack conversations.replies failed: {Status} {Body}", res.StatusCode, body);
            return null;
        }
    ...
    private FetchedMessage? ExtractMessage(string body)
    {
        try
        {
            using var doc = JsonDocument.Parse(body);
                ...
                && messages.GetArrayLength() > 0)
            {
                ...
                return new FetchedMessage(text, threadTs);
            }
        }
        catch (JsonException ex)
        {
